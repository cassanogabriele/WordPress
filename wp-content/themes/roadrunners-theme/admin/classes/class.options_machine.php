...
class Options_Machine {
		...
		foreach ($options as $value) {
			...
			switch ( $value['type'] ) {
				...
				case 'slider':
					...
					$output .= '</ul>';
					$output .= '<a href="#" class="button slide_add_button">Add New Image</a></div>';
				...
				case 'backup':
					...
					$output .= '<p><strong>'. __( 'Last Backup : ' , 'roadrunners' ).'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
	...
	/**
	 * Drag and drop slides manager (modified for CHIMERA)
	 *
	 * @uses get_theme_mod()
	 *
	 * @access public
	...
	public static function optionsframework_slider_function($id,$std,$oldorder,$order){
		...
		if (!empty($val['title'])) {
			...
			$slider .= '<li><div class="slide_header"><strong>Image '.$order.'</strong>';
		}
